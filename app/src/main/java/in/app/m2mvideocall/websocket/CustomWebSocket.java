package in.app.m2mvideocall.websocket;
...
import in.app.m2mvideocall.SessionActivity;
import in.app.m2mvideocall.constants.JsonConstants;
import in.app.m2mvideocall.observers.CustomSdpObserver;
import in.app.m2mvideocall.openvidu.LocalParticipant;
import in.app.m2mvideocall.openvidu.Participant;
...
public class CustomWebSocket extends AsyncTask<SessionActivity, Void, Void> implements WebSocketListener {
    ...
    public CustomWebSocket(Session session, String openviduUrl, SessionActivity activity, DisplayMetrics displayMetrics) {
    ...
    public void publishVideo(SessionDescription sessionDescription) {
        ...
        publishVideoParams.put("videoDimensions", "{\"width\":"+displayMetrics.widthPixels+", \"height\":240}"); //320
    ...
    public void publishNoVideo(SessionDescription sessionDescription) {
        Map<String, String> publishVideoParams = new HashMap<>();
        ...
        publishVideoParams.put("typeOfVideo", "CAMERA");
        publishVideoParams.put("videoDimensions", "{\"width\":0\", \"height\":0}"); //320
        publishVideoParams.put("sdpOffer", sessionDescription.description);
        this.ID_PUBLISHVIDEO.set(this.sendJson(JsonConstants.PUBLISHVIDEO_METHOD, publishVideoParams));
    }
    ...
    private void handleServerEvent(JSONObject json) throws JSONException {
        if (!json.has(JsonConstants.PARAMS)) {
            Log.d(TAG, "handleServerEvent No params " + json.toString());
        } else {
    ...
    private void participantLeftEvent(JSONObject params) throws JSONException {
        ...
        Runnable myRunnable = () -> session.removeView(remoteParticipant);
    ...
    private String getWebSocketAddress(String openviduUrl) {
        try {
            ...
            Log.d(TAG, "getWebSocketAddress --> "+url.getHost()+" --- "+url.getPort());
    ...
    protected Void doInBackground(SessionActivity... sessionActivities) {
        try {
            ...
            Log.d(getClass().getName(), "getWebSocketAddress "+getWebSocketAddress(openviduUrl));
